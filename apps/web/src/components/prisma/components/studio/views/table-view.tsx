...
export function TableView(props: {
  ...
  const [selectedCell, setSelectedCell] = useState<TableCellCoordinate | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadRows() {
      ...
      const [error, result] = await adapter.query(
        {
          filter: undefined,
          fullTableSearchTerm: querySearchTerm.length > 0 ? querySearchTerm : undefined,
      ...
      const nextTotalRowCount = parseRowCount(result?.filteredRowCount, nextRows.length);
  ...
  function toggleRowSelection(rowIndex: number) {
    setSelectedRowIndexes((currentSelection) => {
      ...
      return [...currentSelection, rowIndex].sort(
        (leftIndex, rightIndex) => leftIndex - rightIndex,
      );
    });
  }

  function handleRowMarkerClick(event: ReactMouseEvent<HTMLButtonElement>, rowIndex: number) {
                  ...
                  {!activeTable ? (
                    <TableRow>
                      <TableCell
                        ...
                        className="h-full px-4 py-3 align-top text-sm text-muted-foreground"
                      >
                        {isIntrospecting ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={columns.length + 1}
                        className="h-full px-4 py-3 align-top text-sm text-red-400"
                      >
                        {rowsError}
                      </TableCell>
                    </TableRow>
                              ...
                              >
                                ...
                                >
                                  <PopoverTrigger
                                    ...
                                    onClick={(event) => {
                                      event.stopPropagation();
                                      handleCellClick(rowIndex, column.key, row[column.key]);
                                    }}
                                  >
...
function parseCellEditorValue(args: { draftValue: string; typeGroup: string | undefined }):
  | {
      success: true;
      value: unknown;
    }
