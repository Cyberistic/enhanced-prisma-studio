export const FULL_TABLE_SEARCH_TIMEOUT_MS = 5000;
...
export const FULL_TABLE_SEARCH_TIMEOUT_MESSAGE = "Search timed out after 5 seconds. This kind of search is expensive, and your table might be too large.";
...
const DATETIME_PARTIAL_PATTERN = /^(\d{4})-(\d{2})-(\d{2})(?:[T ]?([01]\d|2[0-3])(?::([0-5]\d)(?::([0-5]\d)(?:\.(\d{1,3}))?)?)?)(?:[Zz])?$/;
const TIME_PATTERN = /^([01]\d|2[0-3])(?::([0-5]\d)(?::([0-5]\d)(?:\.(\d{1,6}))?)?)?$/;
export function createFullTableSearchExecutionState() {
    return {
        activeController: null,
        latestRequestId: 0,
    };
}
export function isFullTableSearchRequest(searchTerm) {
    return (searchTerm?.trim().length ?? 0) > 0;
}
export async function executeQueryWithFullTableSearchGuardrails(args) {
...
export function getFullTableSearchExpression(plan, args) {
    const { dialect } = args;
    return (eb) => {
        if (plan.predicates.length === 0) {
            return eb.lit(true);
        }
        return eb.or(plan.predicates.map((predicate) => {
            switch (predicate.kind) {
                case "text-like": {
                    ...
                    return eb(eb.fn("lower", [eb.cast(predicate.column, textCastType)]), "like", predicate.pattern.toLowerCase());
                }
                case "numeric-equals": {
                    return eb(eb.cast(predicate.column, getNumericCastType(dialect)), "=", eb.cast(eb.val(predicate.value), getNumericCastType(dialect)));
                }
                case "boolean-equals": {
...
function getNumericCastType(dialect) {
    if (dialect === "mysql") {
        return "decimal";
    }
    return "numeric";
...
function isTextSearchColumn(column) {
...
function parseBooleanTerm(value) {
...
function parseNumericTerm(value) {
    if (!NUMERIC_PATTERN.test(value)) {
        return null;
    }
    return value;
}
function parseUuidTerm(value) {
    return UUID_PATTERN.test(value) ? value : null;
}
function parseDateOnlyTerm(value) {
...
function parseDateTimeTerm(value) {
