...
import { createFullTableSearchExecutionState, executeQueryWithFullTableSearchGuardrails, } from "../full-table-search";
...
export function createSQLiteAdapter(requirements) {
    ...
    const createSQLiteAdapterError = (args) => createAdapterError({ ...args, adapterSource: "sqlite" });
    ...
    return {
        ...
        async sqlLint(details, options) {
            if (canUseExecutorLintTransport &&
...
function shouldFallbackToExplainLint(error) {
    ...
    return (message.includes("invalid procedure") ||
...
function createIntrospection(args) {
    ...
    return {
        filterOperators,
        query: tablesQuery,
        schemas: tables.reduce((schemas, table) => {
            ...
            const columnsRecord = columns.reduce((columnsRecord, column, index) => {
                 ...
                 */
                const isRowId = datatype.toUpperCase() === "INTEGER" &&
                    pk === 1 &&
