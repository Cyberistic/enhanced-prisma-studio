...
export function getTablesQuery(requirements) {
  return compile(
    getSQLiteBuilder(requirements)
      .selectFrom(expressionBuilder().fn("pragma_table_list", []).as("tl"))
      ...
      .select((eb) => [
        jsonArrayFrom(
          eb
            .selectFrom(eb.fn("pragma_table_xinfo", ["tl.name"]).as("txi"))
            .leftJoin(
              eb.fn("pragma_foreign_key_list", ["tl.name"]).as("fkl"),
              "fkl.from",
              "txi.name",
            )
