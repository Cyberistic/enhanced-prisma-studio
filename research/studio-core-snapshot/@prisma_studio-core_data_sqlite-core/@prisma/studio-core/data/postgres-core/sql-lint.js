...
export function validateSqlForLint(sql) {
...
function getStatementFirstKeyword(statement) {
...
function getPostgresErrorPosition(error) {
    const withPosition = error;
...
function getPostgresErrorSeverity(error) {
    const withSeverity = error;
...
function getDiagnosticRange(args) {
    ...
    const from = Math.min(maxFrom, Math.max(0, (Number.isFinite(position) ? position : 1) - 1));
    if (sqlLength === 0) {
        return { from: 0, to: 0 };
    }
