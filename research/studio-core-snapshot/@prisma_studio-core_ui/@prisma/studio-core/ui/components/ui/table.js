...
const Table = React.forwardRef(({ className, containerProps, ...props }, ref) => {
    const { className: containerClassName, ...resolvedContainerProps } = containerProps ?? {};
    return (<div className={cn("relative w-full overflow-auto flex items-start h-full", containerClassName)} {...resolvedContainerProps}>
        <table ref={ref} className={cn("w-full caption-bottom text-sm", className)} {...props}/>
      </div>);
});
Table.displayName = "Table";
const TableHeader = React.forwardRef(({ className, ...props }, ref) => (<thead ref={ref} className={cn(className)} {...props}/>));
...
const TableHead = React.forwardRef(({ className, ...props }, ref) => (<th ref={ref} className={cn("h-10 px-2 text-left align-middle font-medium text-muted-foreground", className)} {...props}/>));
TableHead.displayName = "TableHead";
const TableCell = React.forwardRef(({ className, ...props }, ref) => (<td ref={ref} className={cn("align-middle", className)} {...props}/>));
