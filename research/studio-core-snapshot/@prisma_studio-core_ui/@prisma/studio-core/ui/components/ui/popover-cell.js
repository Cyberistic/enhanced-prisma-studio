...
const PopoverCell = ({ children, onOpenChange, open: controlledOpen, ...props }) => {
    ...
    return (<PopoverCellContext.Provider value={{
...
const PopoverCellTrigger = forwardRef(({ children, onDoubleClick, className, ...props }, ref) => {
    ...
    return (<PopoverPrimitive.Trigger className={cn("w-full h-full cursor-pointer", className)} onClick={(event) => {
            onClick?.(event);
...
const PopoverCellContent = forwardRef(({ className, align = "center", onInteractOutside, sideOffset = 4, ...props }, ref) => {
    const context = useContext(PopoverCellContext);
    return (<PopoverPrimitive.Portal>
        <div className="ps">
          <PopoverPrimitive.Content {...props} align={align} className={cn("flex flex-col gap-2 z-50 border border-border bg-popover text-popover-foreground shadow-md outline-none", className)} data-studio-cell-editor="true" onCloseAutoFocus={(event) => {
            event.preventDefault();
    ...
    onEscapeKeyDown={(event) => event.preventDefault()} ref={ref} sideOffset={sideOffset}/>
        </div>
