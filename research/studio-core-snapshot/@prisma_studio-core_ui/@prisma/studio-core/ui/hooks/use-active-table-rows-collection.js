...
export function useActiveTableRowsCollection(query) {
    ...
    const { metadata: { activeTable }, } = useNavigation();
    const sortKey = useMemo(() => getSortKey(sortOrder), [sortOrder]);
    const filterKey = useMemo(() => `${getFilterKey(filter)}::${fullTableSearchTerm ?? ""}`, [filter, fullTableSearchTerm]);
    const queryScopeKey = useMemo(() => getQueryScopeKey(activeTable, {
        fullTableSearchTerm,
        pageIndex,
        pageSize,
        sortOrder,
    ...
    const filteredRowCountKey = useMemo(() => getFilteredRowCountKey(activeTable, {
        filter,
        fullTableSearchTerm,
    }), [activeTable, filter, fullTableSearchTerm]);
    const queryKey = useMemo(() => activeTable
        ? [
            "schema",
            activeTable.schema,
            "table",
        ...
        : null, [activeTable, filterKey, pageIndex, pageSize, sortKey]);
        ...
        return studio.getOrCreateRowsCollection(queryScopeKey, () => {
            return createCollection(queryCollectionOptions({
                ...
                queryFn: async ({ signal }) => {
                    ...
                    upsertFilteredRowCount(filteredRowCountKey, result.filteredRowCount, tableQueryMetaCollection);
    ...
    const isQueryFetching = useIsFetching(queryKey ? { queryKey, exact: true } : undefined, queryClient);
    const filteredRowCount = tableQueryMetaCollection.get(filteredRowCountKey)?.filteredRowCount ??
        Infinity;
