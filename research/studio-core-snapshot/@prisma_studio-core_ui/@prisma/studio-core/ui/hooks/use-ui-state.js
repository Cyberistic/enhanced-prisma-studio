...
export function useUiState(key, initialValue, options = {}) {
  ...
  const uiLocalStateCollection = studioContext?.uiLocalStateCollection ?? fallbackUiStateCollection;
  ...
  return [
    cleanupOnUnmount ? volatileValue : (stateRow?.value ?? cloneValue(initialValue)),
    setValue,
    resetValue,
  ];
}
