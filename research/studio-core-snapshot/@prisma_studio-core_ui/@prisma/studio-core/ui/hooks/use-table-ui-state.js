...
import { cloneEditingFilter, createDefaultFilter, createEditingFilterFromApplied, } from "./filter-utils";
import { useNavigation } from "./use-navigation";
function createDefaultTableUiState(scopeKey, defaults) {
    return {
        id: scopeKey,
        editingFilter: cloneEditingFilter(defaults?.editingFilter ??
...
export function useTableUiState(defaults) {
    ...
    const { data: tableUiState } = useLiveQuery((q) => {
        if (!scopeKey) {
            return undefined;
        }
        return q
    ...
    const resolvedState = !scopeKey || tableUiState
        ? tableUiState
        : createDefaultTableUiState(scopeKey, defaults);
    const updateTableUiState = useCallback((updater) => {
        if (!scopeKey) {
            return;
        }
