...
export function createEditingFilterFromApplied(filterGroup) {
    return JSON.parse(JSON.stringify(filterGroup));
}
export function cloneEditingFilter(filterGroup) {
    return JSON.parse(JSON.stringify(filterGroup));
}
export function createEditingColumnFilter(column) {
    return {
        after: "and",
        column,
        draftValue: "",
...
export function createEditingSqlFilter(sql = "") {
    return {
        after: "and",
        id: short.generate(),
        kind: "SqlFilter",
...
export function createAppliedFilterFromEditing(filterGroup, columns) {
    return {
        ...filterGroup,
        filters: filterGroup.filters.flatMap((filter) => {
            ...
            if (filter.kind === "SqlFilter") {
                if (getEditingFilterSyntaxIssue(filter, columns ?? {})) {
                    return [];
                }
...
export function countFiltersRecursive(filterGroup) {
    let count = 0;
...
export function getSupportedFilterOperatorsForColumn(column, availableOperators) {
    const baseOperators = ["=", "!=", "is", "is not"];
    const supportedOperators = column.datatype.isArray
...
export function isFilterOperatorSupportedForColumn(column, operator) {
    return getSupportedFilterOperatorsForColumn(column).includes(operator);
}
export function getEditingFilterSyntaxIssue(filter, columns) {
    if (filter.kind === "SqlFilter") {
    ...
    switch (getScalarColumnSyntaxKind(column)) {
        ...
        case "enum":
            if (EQUALITY_OPERATORS.has(filter.operator) &&
...
export function getEditingFilterIssue(filter, columns) {
...
export function isEditingFilterSyntacticallyValid(filter, columns) {
    return getEditingFilterSyntaxIssue(filter, columns) == null;
}
export function attachAiSourceToEditingFilter(filterGroup, query) {
...
export function mergeEditingFilterUiMetadata(args) {
    ...
    const aiSourceByFilterId = new Map();
    function collectPreviousFilterMetadata(filterGroup) {
        for (const filter of filterGroup.filters) {
            if (filter.kind === "FilterGroup") {
    ...
    function applyPreviousFilterMetadata(filterGroup) {
        return {
            ...filterGroup,
            filters: filterGroup.filters.map((filter) => {
                if (filter.kind === "FilterGroup") {
...
function getEditingFilterRawValue(filter, column) {
...
function isValidJsonArrayLiteral(value) {
    if (value.length === 0) {
        return false;
    }
    try {
...
function isValidJsonLiteral(value) {
    if (value.length === 0) {
        return false;
    }
    try {
...
function isValidNumberLiteral(value) {
    if (value.length === 0) {
        return false;
    }
...
function isValidTimeLiteral(value) {
    ...
    return /^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d{1,6})?)?(?:Z|[+-](?:[01]\d|2[0-3]):[0-5]\d)?$/.test(value);
}
function getSupportedScalarOperatorsForKind(kind) {
    switch (kind) {
        case "datetime":
        case "numeric":
        case "time":
...
function getScalarColumnSyntaxKind(column) {
...
function supportsTextSearchOnArray(column) {
    return ["enum", "text"].includes(getScalarColumnSyntaxKind(asScalarColumn(column)));
}
function getUnsupportedOperatorIssue(column, operator) {
...
function asScalarColumn(column) {
    return {
        ...column,
        datatype: {
            ...column.datatype,
...
function isUuidLikeColumn(column) {
    return getNormalizedTypeName(column) === "uuid";
}
function getNormalizedTypeName(column) {
    return column.datatype.name.toLowerCase();
}
function isValidUuidLiteral(value) {
    if (value.length === 0) {
        return false;
    }
