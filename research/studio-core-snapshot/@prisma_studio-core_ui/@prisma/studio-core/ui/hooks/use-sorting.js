...
export function useSorting() {
    const { metadata: { activeTable }, pageIndexParam, setPageIndexParam, sortParam, setSortParam, } = useNavigation();
    // Parse sorting from URL parameter
    const parseSorting = (sortParam) => {
        if (!sortParam)
            return [];
        try {
    ...
    const handleSortingChange = useCallback((newState) => {
        ...
        const sortString = resolvedState.length > 0
            ? resolvedState
...
function getPrimaryKeyDefaultSorting(activeTable) {
    if (!activeTable) {
        return [];
    }
