...
import { attachAiSourceToEditingFilter, createEditingColumnFilter, createEditingSqlFilter, getEditingFilterSyntaxIssue, getSupportedFilterOperatorsForColumn, isFilterOperator, } from "./filter-utils";
...
export function buildAiFilterPrompt(args) {
    const { filterOperators, now = new Date(), request, table, timeZone } = args;
    const availableOperators = filterOperators && filterOperators.length > 0
        ? filterOperators
...
export function buildAiFilterCorrectionPrompt(args) {
    ...
    const issueLines = issues.length > 0
        ? issues.map((issue) => {
            const prefix = issue.filterIndex != null
...
export async function resolveAiFiltering(args) {
    ...
    const retryResponseText = await aiFilter(buildAiFilterCorrectionPrompt({
        filterOperators,
    ...
    if (retryResult.filterGroup.filters.length === 0 &&
...
export function parseAiFilterResponseToEditingFilter(args) {
    ...
    const availableOperators = new Set(filterOperators && filterOperators.length > 0
        ? filterOperators
        : DEFAULT_FILTER_OPERATORS);
    ...
    for (const [filterIndex, candidate] of (parseResult.filters ?? []).entries()) {
        ...
        if (typeof candidate.operator !== "string" ||
            ...
            issues.push({
                ...
                operator: typeof candidate.operator === "string"
                    ? candidate.operator
                    : undefined,
                responseText,
                value: candidate.value === undefined
...
function createEmptyEditingFilterGroup() {
    return {
        after: "and",
        filters: [],
...
function parseAiFilterResponse(responseText) {
...
function serializeAiFilterValue(value) {
    if (value === undefined) {
        return "";
    }
    if (value === null) {
...
function formatPromptLocalDateTime(now, timeZone) {
