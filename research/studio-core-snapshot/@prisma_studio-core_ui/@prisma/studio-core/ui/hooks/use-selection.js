...
export function useSelection(data) {
    ...
    const rowSelectionState = useMemo(() => tableUiState?.rowSelectionState ?? {}, [tableUiState?.rowSelectionState]);
    ...
    const setRowSelectionState = useCallback((updater) => {
        updateTableUiState((draft) => {
    ...
    function deleteSelection() {
        const rows = data?.rows.filter((row) => {
            return rowSelectionState[row.__ps_rowid];
        });
        mutate(rows ?? [], {
            onSuccess() {
