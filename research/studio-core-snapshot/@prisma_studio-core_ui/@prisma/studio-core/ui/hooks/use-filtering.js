...
import { countFiltersRecursive, createAppliedFilterFromEditing, createEditingFilterFromApplied, defaultFilter, mergeEditingFilterUiMetadata, } from "./filter-utils";
...
export function useFiltering(columns) {
    ...
    const editingFilterDefaults = useMemo(() => createEditingFilterFromApplied(appliedFilter), [appliedFilter]);
    const setAppliedFilter = useCallback((filter) => void setFilterParam(JSON.stringify(filter)), [setFilterParam]);
    ...
    const setEditingFilter = useCallback((filter) => {
        updateTableUiState((draft) => {
            draft.editingFilter = filter;
        });
    }, [updateTableUiState]);
    const applyEditingFilter = useCallback((filter = editingFilter) => {
    ...
    useEffect(() => {
        ...
        setEditingFilter(mergeEditingFilterUiMetadata({
