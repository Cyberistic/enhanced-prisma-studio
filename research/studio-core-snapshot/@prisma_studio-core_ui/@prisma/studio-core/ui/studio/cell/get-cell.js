...
import { DEFAULT_ARRAY_DISPLAY, DEFAULT_BOOLEAN, DEFAULT_JSON, DEFAULT_NUMERIC, getDate0, } from "../../../data/defaults";
import { DefaultValueCell } from "./DefaultValueCell";
import { HighlightSearchMatch } from "./highlight-search-match";
import { StringCell } from "./StringCell";
export function getCell(props) {
    ...
    if (isArray || group === "json") {
        return (<HighlightSearchMatch searchTerm={searchTerm} text={isObjectType(value) ? JSON.stringify(value, null, 2) : String(value)}/>);
    }
    if (group === "string") {
        if (value === "") {
            return <EmptyStringCell />;
...
function EmptyStringCell() {
    return (<span className="italic text-muted-foreground select-none">
      (empty string)
    </span>);
}
