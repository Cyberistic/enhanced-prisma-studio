...
export function Link(props) {
    ...
    return (<RelationLink createUrl={createUrl} filterColumn={fkColumn} filterValue={cell.getValue()} introspection={introspection} targetSchema={fkSchema} targetTable={fkTable}/>);
}
export function RelationLink(props) {
    ...
    return (<Button aria-label={`Open ${table.name}`} className="shrink-0" onMouseDown={(e) => e.stopPropagation()} size={"xs"} variant={"outline"} onClick={(e) => e.stopPropagation()} asChild>
      <a href={createUrl({
