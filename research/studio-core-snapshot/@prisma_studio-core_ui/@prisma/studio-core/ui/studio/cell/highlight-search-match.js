...
const DATETIME_HIGHLIGHT_TERM_PATTERN = /^(\d{4}-\d{2}-\d{2})([Tt ]?)([01]\d|2[0-3])((?::[0-5]\d(?::[0-5]\d(?:\.\d{1,6})?)?)?)([Zz]?)$/;
export function getSearchMatchSegments(text, searchTerm) {
...
function getMatchRanges(text, searchTerm) {
...
function getDateTimeTermVariants(searchTerm) {
    ...
    const separatorVariants = separator === "" ? ["T", " "] : separator === " " ? ["T", ""] : [" ", ""];
...
export function HighlightSearchMatch(props) {
    ...
    return (<>
      {segments.map((segment, index) => {
            ...
            return (<mark key={key} className="rounded-sm bg-yellow-200/80 text-current dark:bg-yellow-300/60" data-search-match="true">
            {segment.text}
          </mark>);
        })}
    </>);
