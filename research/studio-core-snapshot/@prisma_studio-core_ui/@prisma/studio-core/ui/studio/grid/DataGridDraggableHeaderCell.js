...
export const DataGridDraggableHeaderCell = (props) => {
    const { children, className, header, style: commonStyle, table, ...tableHeadProps } = props;
    const { attributes, listeners, transform, transition, setNodeRef, isDragging, } = useSortable({
        id: header.id,
    });
    const shouldPreviewReorder = Boolean(table.options.meta?.isColumnReorderPreviewEnabled);
    ...
    const style = {
        ...
        cursor: "grab",
        touchAction: "none",
        transform: [commonStyle?.transform, dragTransform].filter(Boolean).join(" ") ||
            undefined,
        transition: shouldSuppressReorderTransform ? undefined : transition,
    };
    const dragClass = cn(className, "p-0", isDragging && "z-50! opacity-0");
    return (<TableHead ref={setNodeRef} {...tableHeadProps} {...attributes} {...listeners} style={style} className={dragClass}>
