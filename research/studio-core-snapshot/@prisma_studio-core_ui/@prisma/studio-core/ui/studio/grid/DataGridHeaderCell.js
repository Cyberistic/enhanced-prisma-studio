...
function HeaderTooltipIcon(props) {
...
export function DataGridHeaderCell({ column }) {
    const { datatype, fkColumn, fkSchema, fkTable, isAutoincrement, isComputed, isRequired, name, pkPosition, } = column;
    return (<div className="flex min-w-0 items-center gap-1 overflow-hidden">
      {pkPosition != null && (<HeaderTooltipIcon tooltip={<span>Primary key</span>}>
          <KeyRound size={12} className="text-amber-500"/>
        </HeaderTooltipIcon>)}
      {fkColumn != null && (<HeaderTooltipIcon tooltip={<span>
              Foreign key - references{" "}
              {[fkSchema, fkTable, fkColumn].filter(Boolean).join(".")}
            </span>}>
          <KeyRound size={12} className="text-primary"/>
        </HeaderTooltipIcon>)}
      {isAutoincrement && (<HeaderTooltipIcon tooltip={<span>Auto-increment</span>}>
          <ArrowUp10 size={12} className="text-muted-foreground"/>
        </HeaderTooltipIcon>)}
      {isComputed && (<HeaderTooltipIcon tooltip={<span>Computed</span>}>
          <Calculator size={12} className="text-muted-foreground"/>
        </HeaderTooltipIcon>)}
      {isRequired && (<HeaderTooltipIcon tooltip={<span>
