export function computeColumnVirtualizationWindow(args) {
    ...
    const startOffsets = new Array(columnCount);
    const endOffsets = new Array(columnCount);
    let runningOffset = 0;
    ...
    const windowEnd = Math.max(windowStart, sanitizedScrollLeft + sanitizedViewportWidth + sanitizedOverscan);
    let startIndex = 0;
...
function fullWindow(columnWidths) {
    return {
        enabled: false,
        startIndex: 0,
