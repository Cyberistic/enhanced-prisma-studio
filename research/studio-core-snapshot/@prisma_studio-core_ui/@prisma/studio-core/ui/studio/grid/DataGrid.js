...
function captureGridLayoutAnimationSnapshot(tableElement) {
  ...
  const elements = tableElement.querySelectorAll(
...
export function DataGrid(props) {
  ...
  const [columnOrder, setColumnOrder] = useUiState(
    `datagrid:${gridScope}:column-order`,
    DEFAULT_COLUMN_ORDER,
  );
  const [columnPinning, setColumnPinning] = useUiState(
    `datagrid:${gridScope}:column-pinning`,
    DEFAULT_COLUMN_PINNING,
  );
  const [columnSizing, setColumnSizing] = useUiState(
  ...
  const [selectionState, setSelectionState] = useUiState(
