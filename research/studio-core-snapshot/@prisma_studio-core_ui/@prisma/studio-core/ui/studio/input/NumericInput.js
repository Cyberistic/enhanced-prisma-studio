...
export function NumericInput(props) {
    ...
    return (<div onKeyDown={handleKeyDown}>
      <div className="flex flex-row items-center h-(--studio-cell-height) gap-4 py-0">
        ...
        <input aria-required={isRequired} className="cell-input-leading appearance-none w-full border-none outline-none shadow-none bg-transparent focus-visible:ring-0 resize-none px-(--studio-cell-spacing)" disabled={readonly} inputMode="numeric" lang="en_EN" onChange={(event) => setCurrentValue(event.target.value)} pattern="\d*" required={isRequired} step="any" type="text" value={currentValue}/>
      </div>
