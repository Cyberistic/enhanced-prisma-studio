...
export function EnumInput(props) {
    ...
    const [currentValue, setCurrentValue] = useUiState(inputStateKey, value == null ? value : String(value), { cleanupOnUnmount: true });
    ...
    const getClassName = (option) => cn(badgeVariants({
    ...
    return (<div onKeyDown={handleKeyDown}>
      <div className="flex items-center gap-2 h-(--studio-cell-height) px-2 py-0 w-full text-sm text-muted-foreground">
        {currentValue === null ? (<span className="italic">(NULL)</span>) : currentValue === undefined ? (<span className="italic">(default value)</span>) : (currentValue)}
      </div>
      <div className="flex flex-row flex-wrap gap-2 p-2 min-w-(--radix-popover-trigger-width) max-w-[300px] w-full border-t border-table-border overflow-y-scroll max-h-32">
        {nullable ? (<button className={getClassName(null)} disabled={readonly} key="__PS_NULL__" onClick={() => setCurrentValue(null)}>
            <span className="italic">(NULL)</span>
          </button>) : null}
        {context === "insert" && defaultValue != null ? (<button className={getClassName(undefined)} disabled={readonly} key="__PS_DEFAULT_VALUE__" onClick={() => setCurrentValue(undefined)}>
            <span className="italic">(default value)</span>
          </button>) : null}
        {options.map((option) => (<button className={getClassName(option)} disabled={readonly} key={option} onClick={() => setCurrentValue(option)}>
            {option}
          </button>))}
      </div>
      <InputActions disabled={readonly} onCancel={handleCancel} onSave={handleSave} showSave={showSaveAction}/>
    </div>);
}
