...
export function BooleanInput(props) {
    ...
    const valueWithDefaults = value == null
        ? isRequired && !fkColumn
            ? DEFAULT_BOOLEAN
            : "indeterminate"
        : Boolean(value);
    const inputStateKey = useStableUiStateKey("boolean-input");
    const [inputValue, setInputValue] = useUiState(inputStateKey, valueWithDefaults, { cleanupOnUnmount: true });
    ...
    return (<div onKeyDown={handleKeyDown}>
      <div className="flex flex-row items-center h-(--studio-cell-height) gap-4 py-0 px-2">
        <Checkbox aria-checked={inputValue === "indeterminate" ? "mixed" : inputValue} aria-disabled={readonly} checked={inputValue} disabled={readonly} id="boolean-input" onCheckedChange={() => setInputValue((prev) => !isRequired && prev === true ? "indeterminate" : !prev)}/>
