...
export function DateInput(props) {
  ...
  const [currentMonth, setCurrentMonth] = useUiState(
    `${statePrefix}:current-month`,
    initialSelectedDate,
    { cleanupOnUnmount: true },
  );
  const [hourValue, setHourValue] = useUiState(
    `${statePrefix}:hour`,
    dayjs(initialSelectedDate).format("HH"),
    { cleanupOnUnmount: true },
  );
  const [minuteValue, setMinuteValue] = useUiState(
    `${statePrefix}:minute`,
    dayjs(initialSelectedDate).format("mm"),
    { cleanupOnUnmount: true },
  );
  const [secondValue, setSecondValue] = useUiState(
    `${statePrefix}:second`,
  ...
  const [yearValue, setYearValue] = useUiState(
    `${statePrefix}:year`,
