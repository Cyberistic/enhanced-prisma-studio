...
export function DateInput(props) {
    ...
    const valueAsString = value == null
        ? isRequired && !fkColumn
            ? getDate0(format)
            : ""
        : String(value);
    ...
    const [selectedDate, setSelectedDate] = useUiState(`${statePrefix}:selected-date`, initialSelectedDate, { cleanupOnUnmount: true });
    const [currentMonth, setCurrentMonth] = useUiState(`${statePrefix}:current-month`, initialSelectedDate, { cleanupOnUnmount: true });
    const [hourValue, setHourValue] = useUiState(`${statePrefix}:hour`, dayjs(initialSelectedDate).format("HH"), { cleanupOnUnmount: true });
    const [minuteValue, setMinuteValue] = useUiState(`${statePrefix}:minute`, dayjs(initialSelectedDate).format("mm"), { cleanupOnUnmount: true });
    const [secondValue, setSecondValue] = useUiState(`${statePrefix}:second`, dayjs(initialSelectedDate).format("ss"), { cleanupOnUnmount: true });
    const { handleOnChange: baseHandleOnChange, setValue, value: inputValue, } = useInput({
    ...
    const { handleCancel, handleKeyDown, handleSave } = usePopoverActions({
        onNavigate,
        onSave: () => {
            const formattedInputValue = !inputValue && isRequired && !fkColumn
                ? getDate0(format)
    ...
    const [yearValue, setYearValue] = useUiState(`${statePrefix}:year`, currentMonth.getFullYear().toString(), { cleanupOnUnmount: true });
    // TODO: simplify this function
    const updateDateFromInputs = (newDateParts) => {
    ...
    const handleYearChange = (e) => {
        ...
        if (newYear >= MIN_JS_DATE.getFullYear() &&
    ...
    const handleTimeChange = (part, value) => {
        ...
        if (part === "minute")
            setMinuteValue(value.padStart(2, "0"));
        if (part === "second")
            setSecondValue(value.padStart(2, "0"));
        // Update only if the value is a complete number for that part
        const isCompleteHour = part === "hour" &&
            value.length >= 1 &&
            numericValue >= 0 &&
            numericValue <= 23;
        const isCompleteMinute = part === "minute" &&
            value.length >= 1 &&
            numericValue >= 0 &&
            numericValue <= 59;
        const isCompleteSecond = part === "second" &&
            value.length >= 1 &&
            numericValue >= 0 &&
            numericValue <= 59;
        if ((part === "hour" && isCompleteHour) ||
    ...
    <div onKeyDown={handleKeyDown}>
      <div className="flex items-center pr-(--studio-cell-spacing)">
        <Input aria-required={isRequired} className="cell-input-leading border-none shadow-none focus-visible:ring-0 resize-none px-(--studio-cell-spacing)" onChange={handleOnChange} placeholder={format} required={isRequired} type="text" value={inputValue}/>
      </div>
      <div className="flex flex-col gap-0 p-2 border-t border-table-border">
        <div className="flex items-center justify-between mb-1 gap-2">
          <Select onValueChange={(value) => updateDateFromInputs({ month: parseInt(value) })} value={currentMonthValue}>
            <SelectTrigger className="text-xs data-[state=open]:border-primary shadow-none">
              <SelectValue placeholder="Month"/>
            </SelectTrigger>
            <SelectContent>
              {MONTHS.map((month, index) => (<SelectItem key={index} value={String(index)}>
                  {month}
                </SelectItem>))}
            </SelectContent>
          </Select>
          <Input className="text-xs w-20 text-right shadow-none" max={MAX_JS_DATE.getFullYear()} min={MIN_JS_DATE.getFullYear()} onChange={handleYearChange} placeholder="YYYY" type="number" value={yearValue}/>
        </div>
        <Calendar captionLayout="dropdown" className="w-full" disabled={readonly} endMonth={MAX_JS_DATE} mode="single" month={currentMonth} onMonthChange={setCurrentMonth} onSelect={handleCalendarSelect} selected={selectedDate} startMonth={MIN_JS_DATE}/>
        {hasTimePrecision && (<div className="mt-1">
            <div className="flex items-center justify-start gap-1">
              <Input className="text-xs w-16 text-right shadow-none" type="number" value={hourValue} onChange={(e) => handleTimeChange("hour", e.target.value)} placeholder="HH" min={0} max={23} maxLength={2} disabled={readonly}/>
              <span className="text-xs text-muted-foreground">:</span>
              <Input className="text-xs w-16 text-right shadow-none" type="number" value={minuteValue} onChange={(e) => handleTimeChange("minute", e.target.value)} placeholder="mm" min={0} max={59} maxLength={2} disabled={readonly}/>
              <span className="text-xs text-muted-foreground">:</span>
              <Input className="text-xs w-16 text-right shadow-none " type="number" value={secondValue} onChange={(e) => handleTimeChange("second", e.target.value)} placeholder="ss" min={0} max={59} maxLength={2} disabled={readonly}/>
              {/* TODO: milliseconds. */}
            </div>
      ...
      <InputActions disabled={readonly} onCancel={handleCancel} onSave={handleSave} saveText={!inputValue
