import { AlertCircle, CheckCircle, ChevronDown, ChevronRight, Copy, TriangleAlertIcon, } from "lucide-react";
...
export function OperationEventEntry({ event }) {
    const [isQueryExpanded, setIsQueryExpanded] = useUiState(`operation-event:${event.eventId}:query-expanded`, false, { cleanupOnUnmount: true });
    ...
    return (<div className="flex flex-col gap-2">
      <Card className={cn("w-full rounded-sm border-ring/20 overflow-clip shadow-none")}>
        <CardHeader className={cn("p-3 font-normal", isError
            ? "border-red-500/60 bg-red-500/10"
            : "border-green-500/60 bg-green-500/10")}>
          <CardTitle className="flex items-center justify-between font-mono text-xs">
            <div className="flex items-center gap-2">
              {isError ? (<AlertCircle size={16} className="text-red-500"/>) : (<CheckCircle size={16} className="text-green-500"/>)}
              <span className="">
        ...
        <CardContent className="flex flex-col gap-3 p-3">
          {event.payload.query && (<div data-response-type="query" className="flex flex-col gap-2">
              <div className="flex items-center justify-between relative">
                <button type="button" onClick={() => setIsQueryExpanded(!isQueryExpanded)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground" aria-expanded={isQueryExpanded}>
                  {isQueryExpanded ? (<ChevronDown size={12}/>) : (<ChevronRight size={12}/>)}
                  SQL Query
                </button>
                <Button aria-label="Copy SQL query" variant="outline" size="icon" className="size-6" onClick={() => {
              ...
              <div className={cn("p-3 bg-secondary/50 border border-border rounded-sm text-xs font-mono overflow-x-auto", isQueryExpanded
          ...
          {isError && event.payload.error && (<div data-response-type="error" className="flex flex-col gap-2">
              <div className="flex items-center justify-between relative">
              ...
              {event.payload.error.adapterSource ? (<div className="flex flex-col gap-1">
              ...
              <div className="p-3 bg-secondary/50 border border-red-400/20 rounded-sm text-xs font-mono overflow-x-auto">
                ...
                ? event.payload.error.errors.map((err, index) => (<div key={index}>{err.message}</div>))
