...
export function useActiveTableRowSearch(args) {
    const { scopeKey, searchTerm, setPageIndexParam, setSearchParam, supportsFullTableSearch, } = args;
    const [rowSearchUiState, setRowSearchUiState] = useUiState(supportsFullTableSearch ? `table-row-search:${scopeKey}` : undefined, {
        isOpen: searchTerm.length > 0,
    });
    const [searchInput, setSearchInput] = useState(searchTerm);
    const rowSearchInputRef = useRef(null);
    ...
    const runRowSearch = useCallback((value) => {
