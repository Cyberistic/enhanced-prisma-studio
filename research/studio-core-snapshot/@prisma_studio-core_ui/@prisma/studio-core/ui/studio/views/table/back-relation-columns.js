export function getBackRelationColumns(args) {
    ...
    const candidates = Object.values(introspection.schemas).flatMap((schema) => Object.values(schema.tables).flatMap((sourceTable) => Object.values(sourceTable.columns).flatMap((column) => {
        if (!column.fkColumn ||
            !column.fkTable ||
    ...
    const countsBySourceTable = new Map();
    const countsByDisplayName = new Map();
    candidates.forEach((candidate) => {
        ...
        countsByDisplayName.set(candidate.sourceTable, (countsByDisplayName.get(candidate.sourceTable) ?? 0) + 1);
    });
    return candidates
        .map((candidate) => {
        const sourceTableKey = `${candidate.sourceSchema}.${candidate.sourceTable}`;
...
export function isBackRelationColumnMeta(value) {
