import { ChevronLeft, ChevronRight, FilterIcon, Plus, RefreshCw, Search, Save, Undo2, } from "lucide-react";
...
function resolveCommandQueryMode(args) {
    ...
    if (COMMAND_LOOKUP_LABELS.some((commandLabel) => commandLabel.toLowerCase().startsWith(normalizedQuery))) {
        return {
            kind: "focus",
            payload: "",
        };
...
export function createActiveTableCommandPaletteActions(args) {
    const { canGoToNextPage, canGoToPreviousPage, hasAiFilter, hasStagedChanges, isInsertingDisabled, onDiscardStagedChanges, onFocusFilterWithAi, onFocusSearch, onGoToNextPage, onGoToPreviousPage, onInsertRow, onRefresh, onRunFilterWithAi, onRunSearch, onSaveStagedChanges, saveStagedChangesLabel, } = args;
    ...
    return [
        {
            ...
            shouldShow: (query) => resolveCommandQueryMode({
                label: SEARCH_ROWS_LABEL,
                query,
            }).kind === "focus",
        },
        {
            ...
            icon: Search,
            keywords: ["search", "rows"],
            label: (query) => `Search rows: ${resolveCommandQueryMode({ label: SEARCH_ROWS_LABEL, query }).payload}`,
            onSelect: (query) => onRunSearch(resolveCommandQueryMode({ label: SEARCH_ROWS_LABEL, query }).payload),
            shouldShow: (query) => resolveCommandQueryMode({
                label: SEARCH_ROWS_LABEL,
                query,
            }).kind === "execute",
        ...
        },
        {
            ...
            icon: FilterIcon,
            keywords: ["filter", "ai", "llm", "natural language"],
            label: (query) => `Filter with AI: ${resolveCommandQueryMode({ label: FILTER_WITH_AI_LABEL, query }).payload}`,
            onSelect: (query) => onRunFilterWithAi(resolveCommandQueryMode({ label: FILTER_WITH_AI_LABEL, query })
                .payload),
            shouldShow: (query) => hasAiFilter &&
                resolveCommandQueryMode({
                    label: FILTER_WITH_AI_LABEL,
