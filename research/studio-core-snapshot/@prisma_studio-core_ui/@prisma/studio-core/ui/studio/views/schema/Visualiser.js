...
const IconWithTooltip = ({ iconElement, tooltipText, className }) => (
  <Tooltip>
...
export function SchemaVisualization({ tables, relationships }) {
  ...
  const [nodes, setNodes] = useUiState(`schema-visualizer:${stateScope}:nodes`, initialNodes, {
    cleanupOnUnmount: true,
  });
  const [edges, setEdges] = useUiState(`schema-visualizer:${stateScope}:edges`, initialEdges, {
    cleanupOnUnmount: true,
  });
