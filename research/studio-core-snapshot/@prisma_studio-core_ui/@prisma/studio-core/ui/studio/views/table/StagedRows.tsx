...
export function StagedRows(props: Props) {
  ...
  function updateStagedRow(rowIndex: number, partialRow: Record<string, unknown>) {
  ...
  function removeStagedRow(rowIndex: number): void {
    setStagedRows((prevRows) => [...prevRows.slice(0, rowIndex), ...prevRows.slice(rowIndex + 1)]);
  }
  ...
  const columnDefs = Object.values(parentTable.options.columns).map((column) => {
    return {
      ...column,
      cell({ cell }) {
        return (props: Omit<CellProps, "children" | "ref"> & { rowIndex: number }) => {
          const { rowIndex, ...propz } = props;

          if (column.id === "__ps_select") {
            return (
              <Cell className="hover:bg-transparent" data-select="true" {...propz}>
                <Button
          ...
          if (isBackRelationColumnMeta(column.meta)) {
            return (
              <Cell
                ...
                className={cn(
                  propz.className,
                  isFocused ? focusedStagedCellClassName : stagedCellClassName,
                )}
              ...
              >
                <div className="flex w-full items-center justify-end">
                  <RelationLink
                    ...
                    filterValue={(stagedRows[rowIndex] ?? {})[column.meta.currentColumnName]}
        ...
        <TableRow key={i}>
          {row.getVisibleCells().map((cell, j) => {
            if (typeof cell.column.columnDef.cell === "function") {
              type Cell = ComponentType<Omit<CellProps, "children" | "ref"> & { rowIndex: number }>;
