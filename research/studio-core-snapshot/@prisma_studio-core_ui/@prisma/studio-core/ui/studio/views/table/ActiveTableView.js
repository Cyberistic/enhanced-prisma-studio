...
import { clampFocusedCell, focusedCellsAreEqual, moveFocusedCell, } from "../../grid/focused-cell";
import { getCellSelectionAnchor, getCellSelectionRange, GRID_SELECTION_MACHINE_INITIAL_STATE, transitionGridSelectionMachine, } from "../../grid/selection-state-machine";
...
import { buildCellSelectionExportTable, buildRowSelectionExportTable, buildSelectionExportFilename, downloadSelectionExport, serializeSelectionExport, } from "./selection-export";
...
export function ActiveTableView(_props) {
    const isMutating = Boolean(useIsMutating());
    const { adapter, aiFilter } = useStudio();
    const { metadata: { activeTable }, createUrl, searchParam, setPageIndexParam, setSearchParam, } = useNavigation();
    ...
    const { isInfiniteScrollEnabled, paginationState, setInfiniteScrollEnabled, setPaginationState, } = usePagination();
    const [loadedInfinitePageCount, setLoadedInfinitePageCount] = useState(1);
    const { appliedFilter, editingFilter, setEditingFilter, applyEditingFilter, totalEditingFilters, } = useFiltering(activeTable?.columns);
    const infiniteScrollResetKey = useMemo(() => getInfiniteScrollResetKey({
        activeRowSearchTerm,
    ...
    const { data, isFetching, refetch: refetchActiveTable, } = useActiveTableQuery({
    ...
    const [stableInfiniteData, setStableInfiniteData] = useState(null);
    ...
    useEffect(() => {
        ...
        setStableInfiniteData((previous) => {
            if (previous?.key === infiniteScrollResetKey &&
    ...
    const { deleteSelection, isSelecting, rowSelectionState, setRowSelectionState, } = useSelection(visibleData);
    const { tableUiState, updateTableUiState } = useTableUiState({
        editingFilter,
    });
    const [tableGridFocusRequest] = useUiState(TABLE_GRID_FOCUS_REQUEST_UI_STATE_KEY, {
        requestId: 0,
        tableId: null,
    });
    const [gridSelectionState, setGridSelectionState] = useUiState(selectionScopeKey
        ? `datagrid:${selectionScopeKey}:selection-state`
        : undefined, GRID_SELECTION_MACHINE_INITIAL_STATE);
    const [gridFocusedCell, setGridFocusedCell] = useUiState(selectionScopeKey
        ? `datagrid:${selectionScopeKey}:focused-cell`
        : undefined, null);
    const [gridColumnOrder] = useUiState(selectionScopeKey
        ? `datagrid:${selectionScopeKey}:column-order`
        : undefined, []);
    const [gridColumnPinning] = useUiState(selectionScopeKey
    ...
    const stagedUpdates = useMemo(() => tableUiState?.stagedUpdates ?? [], [tableUiState?.stagedUpdates]);
    const [activeEditorCellKey, setActiveEditorCellKey] = useState(null);
    ...
    const discardConfirmationPrompt = getDiscardConfirmationPrompt(getStagedCellCount({
        stagedRows,
        stagedUpdates,
    }));
    const pendingInfinitePageRowTargetRef = useRef(null);
    const observedInfiniteFetchRef = useRef(false);
    const previousInfiniteScrollResetKeyRef = useRef(infiniteScrollResetKey);
    const setStagedRows = useCallback((updater) => {
    ...
    const setStagedUpdates = useCallback((updater) => {
        updateTableUiState((draft) => {
    ...
    const stagedUpdateMap = useMemo(() => new Map(stagedUpdates.map((stagedUpdate) => [stagedUpdate.rowId, stagedUpdate])), [stagedUpdates]);
    ...
    const discardWiggleAnimationClassName = discardWiggleCount === 0
        ? null
    ...
    const discardWiggleAnimationKey = discardWiggleCount === 0
        ? null
        : discardWiggleCount % 2 === 0
            ? "ps-discard-wiggle-b"
            : "ps-discard-wiggle-a";
    ...
    const readonly = !Object.values(activeTable?.columns ?? {}).some((column) => column.pkPosition != null);
    const isInserting = useIsInserting();
    const insert = useActiveTableInsert();
    const updateMany = useActiveTableUpdateMany();
    const pageCount = getPageCount(visibleData?.filteredRowCount ?? Infinity, paginationState.pageSize);
    const hasMoreInfiniteRows = isInfiniteScrollEnabled &&
    ...
    useLayoutEffect(() => {
        ...
        if (rows.length >= pendingRowTarget ||
            !hasMoreInfiniteRows ||
    ...
    const activeTableId = activeTable
        ? `${activeTable.schema}.${activeTable.name}`
        : null;
    const backRelationColumns = useMemo(() => getBackRelationColumns({
        introspection,
        table: activeTable,
    }), [activeTable, introspection]);
    const defaultColumnIds = useMemo(() => getDefaultTableColumnIds({
        backRelationColumns,
    ...
    const selectionExportColumnIds = useMemo(() => getSelectionExportColumnIds({
    ...
    const editableColumnIds = useMemo(() => getEditableColumnIds(activeTable?.columns, readonly), [activeTable?.columns, readonly]);
    const focusedColumnIds = useMemo(() => getSelectionExportColumnIds({
        columnOrder: gridColumnOrder,
        columnPinning: gridColumnPinning,
        defaultColumnIds: fallbackSelectionExportColumnIds,
    }), [fallbackSelectionExportColumnIds, gridColumnOrder, gridColumnPinning]);
    const editorRows = useMemo(() => buildEditorRows({
        displayRows,
        persistedRows: rows,
        stagedRows,
    }), [displayRows, rows, stagedRows]);
    const resolvedFocusedCell = useMemo(() => clampFocusedCell({
    ...
    const handleInfiniteScrollEnabledChange = useCallback((enabled) => {
        if (hasStagedChanges) {
    ...
    const handleFocusedCellChange = useCallback((focusedCell) => {
    ...
    const handleAiPaletteFilter = useCallback(async (request) => {
        if (!aiFilter || !activeTable) {
            return;
        }
        try {
    ...
    const newStagedRow = useCallback(() => {
        ...
        handleFocusedCellChange(focusedColumnIds[0]
            ? {
    ...
    function buildSerializedSelectionExport(format) {
    ...
    function handleCopySelectionExport(format) {
    ...
    function handleSaveSelectionExport(format) {
        if (!activeTable) {
            return;
        }
        const content = buildSerializedSelectionExport(format);
    ...
    const commandPaletteActions = useMemo(() => createActiveTableCommandPaletteActions({
        canGoToNextPage,
        canGoToPreviousPage,
        hasAiFilter: typeof aiFilter === "function",
        hasStagedChanges,
    ...
    function handleCellInputSubmit(value, column, row) {
        const rowId = String(row.__ps_rowid ?? "");
        if (!rowId) {
            return;
        }
        setStagedUpdates((previous) => upsertStagedCellUpdate(previous, {
    ...
    function handleEditorNavigate(params) {
        ...
        setActiveEditorCellKey(nextTarget
            ? createEditorCellKey({
    ...
    const closePersistedEditor = useCallback((params) => {
        flushSync(() => {
    ...
    const openFocusedCellEditor = useCallback((focusedCell) => {
    ...
    useLayoutEffect(() => {
        if (previousActiveEditorCellKeyRef.current && !activeEditorCellKey) {
            document
                .querySelector('[data-grid-scroll-container="true"]')
                ?.focus({
                preventScroll: true,
            });
            setGridFocusRequestId((previous) => previous + 1);
    ...
    useEffect(() => {
        const handleKeyDown = (event) => {
            ...
            if (event.shiftKey) {
                const selectionStart = getCellSelectionAnchor(gridSelectionState) ??
                ...
                if (selectionStart && selectionEnd) {
                    ...
                    setGridSelectionState((previous) => transitionGridSelectionMachine(previous, {
                        type: "cell.select",
                        end: selectionEnd,
    ...
    function handlePasteSelection(changes) {
        ...
        toast.success(`Bulk paste staged for ${rowChangesById.size} row${rowChangesById.size === 1 ? "" : "s"}`);
    }
    ...
    const concreteColumnDefs = Object.values(activeTable?.columns ?? {})
        ...
        return {
            ...
            header({ table, header }) {
                ...
                <DataGridHeader header={header} column={column} isSortDisabled={hasStagedChanges} onBlockedSortInteraction={triggerDiscardButtonWiggle}/>
            ...
            cell({ cell }) {
                return (props) => {
                    ...
                    const stagedUpdate = stagedUpdateMap.get(rowId);
                    const isCellStaged = Boolean(stagedUpdate &&
                        Object.prototype.hasOwnProperty.call(stagedUpdate.changes, column.name));
                    const isFocused = resolvedFocusedCell?.rowIndex === visualRowIndex &&
                        resolvedFocusedCell.columnId === column.name;
                    if (column.isAutoincrement || column.isComputed) {
                        return (<Cell {...props} className={cn(props.className, isFocused && focusedCellClassName)} data-focused={isFocused || undefined} data-grid-visual-row-index={visualRowIndex} withContextMenu={false}>
                  {getCell({ cell, column, searchTerm: activeRowSearchTerm })}
                </Cell>);
                    }
                    return (<WriteableCell cellComponent={getCell({
                            cell,
                            column,
                            searchTerm: activeRowSearchTerm,
                        })} containerProps={{
                            ...
                            className: cn(props.className, isFocused && isCellStaged
                            ...
                            : null} isEditorOpen={isEditorOpen} linkComponent={Link({
                            cell,
                            column,
                            createUrl,
                            introspection,
                        })} onRequestClose={() => closePersistedEditor({
                            columnId: column.name,
                            editorCellKey,
                            rowIndex: visualRowIndex,
                        })} onRequestOpen={() => {
        ...
        return {
            ...
            cell({ cell }) {
              ...
              <div className="flex h-full w-full items-center justify-end">
                <RelationLink createUrl={createUrl} filterColumn={column.sourceColumn} filterValue={baseRow[column.currentColumnName]} introspection={introspection} targetSchema={column.sourceSchema} targetTable={column.sourceTable}/>
              </div>
            </Cell>);
                };
            },
    ...
    return (<>
      <StudioHeader className={editingFilter.filters.length > 0 ? "border-b-0 pb-2" : undefined} endContent={<Button aria-label="Refresh table" variant="outline" size="icon" onClick={() => void reload()} disabled={isFetching}>
            <RefreshCw data-icon="inline-start" className={cn(isFetching && "animate-spin")}/>
          </Button>}>
        <div className="flex min-w-0 items-center gap-2">
          <ActiveTableRowSearchControl disabled={hasStagedChanges} onBlockedInteraction={triggerDiscardButtonWiggle} rowSearch={rowSearch} supportsFullTableSearch={supportsFullTableSearch}/>
        </div>
        <InlineTableFilterAddButton aiFilter={aiFilter} aiFocusRequestKey={aiFocusRequestKey} applyEditingFilter={applyEditingFilter} disabled={hasStagedChanges} editingFilter={editingFilter} filterOperators={introspection.filterOperators} onBlockedInteraction={triggerDiscardButtonWiggle} setEditingFilter={setEditingFilter} table={activeTable} totalEditingFilters={totalEditingFilters}/>
        <Button variant="outline" className="h-9 px-5" onClick={newStagedRow} disabled={isInserting > 0}>
          Insert row
        </Button>
        {hasSelectionExport && (<DropdownMenu open={isSelectionExportOpen} onOpenChange={(open) => {
            ...
            <DropdownMenuTrigger asChild>
              <Button aria-expanded={isSelectionExportOpen} aria-label="Copy selection as" variant="outline" className={cn("h-9 shrink-0 gap-1.5 px-3 font-sans", isSelectionExportOpen && "bg-accent text-accent-foreground")}>
                <span>copy as</span>
                <ChevronDown className="size-3.5 text-muted-foreground"/>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" side="bottom" className="w-[220px] max-w-[calc(100vw-2rem)] overflow-hidden p-1 font-sans">
              <DropdownMenuCheckboxItem checked={includeSelectionExportHeader} className="rounded-lg font-sans text-sm font-medium" onCheckedChange={(checked) => setIncludeSelectionExportHeader(checked === true)} onSelect={(event) => {
                event.preventDefault();
            }}>
            ...
            ].map((item) => (<DropdownMenuItem key={item.label} className="rounded-lg font-sans text-sm font-medium" onSelect={item.action}>
                    {item.label}
                  </DropdownMenuItem>))}
              </div>
            </DropdownMenuContent>
        ...
        {hasStagedChanges && (<>
            <Button className="h-9 border border-emerald-300 bg-emerald-100 px-5 font-sans text-emerald-950 hover:bg-emerald-200" onClick={() => setSaveDialogOpen(true)}>
              {getSaveStagedRowsLabel(totalStagedRowCount)}
            </Button>
            <Button data-wiggle-animation={discardWiggleAnimationKey
                ? `${discardWiggleAnimationKey}-${discardWiggleCount}`
                : undefined} variant="outline" className={cn("h-9 border-amber-300 bg-amber-100 px-4 font-sans text-amber-950 hover:border-amber-400 hover:bg-amber-200 motion-safe:origin-center motion-safe:will-change-transform", discardWiggleAnimationClassName)} onClick={() => setDiscardDialogOpen(true)}>
              Discard edits
            </Button>
          </>)}
        {isSelecting && selectedRowCount > 0 && (<Button variant="secondary" size="sm" onClick={() => setDeleteDialogOpen(true)}>
            {deleteSelectionLabel}
          </Button>)}
      </StudioHeader>
      <InlineTableFiltersHeaderRow applyEditingFilter={applyEditingFilter} disabled={hasStagedChanges} editingFilter={editingFilter} filterOperators={introspection.filterOperators} onBlockedInteraction={triggerDiscardButtonWiggle} setEditingFilter={setEditingFilter} sqlFilterLint={sqlFilterLint} table={activeTable}/>
      <DataGrid areRowsInViewActionsLocked={hasStagedChanges} canWriteToCell={canWriteToCell} columnDefs={columnDefs} focusScrollContainerKey={gridFocusRequestId || undefined} focusRowIndexOffset={stagedRows.length} focusedCell={resolvedFocusedCell} getBeforeRows={(table) => (<StagedRows activeEditorCellKey={activeEditorCellKey} focusedCell={resolvedFocusedCell} onEditorNavigate={handleEditorNavigate} onFocusedCellChange={handleFocusedCellChange} setActiveEditorCellKey={setActiveEditorCellKey} setStagedRows={setStagedRows} stagedRows={stagedRows} table={table}/>)} hasMoreInfiniteRows={hasMoreInfiniteRows} infiniteScrollEnabled={isInfiniteScrollEnabled} isFetching={isFetching} isProcessing={isMutating} onFocusedCellChange={handleFocusedCellChange} onBlockedRowsInViewAction={triggerDiscardButtonWiggle} onInfiniteScrollEnabledChange={handleInfiniteScrollEnabledChange} onLoadMoreRows={handleLoadMoreRows} onPinnedColumnIdsChange={setPinnedColumnIds} onPaginationChange={setPaginationState} onPasteSelection={handlePasteSelection} onRowSelectionChange={setRowSelectionState} onSortingChange={setSortingState} pageCount={pageCount} paginationState={paginationState} pinnedColumnIds={pinnedColumnIds} rows={displayRows} rowSelectionState={rowSelectionState} selectionScopeKey={selectionScopeKey} sortingState={sortingState}/>
      <BinaryAlertDialog onOpenChange={setDeleteDialogOpen} onPrimaryAction={confirmDeleteSelection} open={isDeleteDialogOpen} primaryLabel="delete" primaryVariant="destructive" prompt={deleteConfirmationPrompt} secondaryLabel="keep" title="Confirm row deletion"/>
      <BinaryAlertDialog onOpenChange={setSaveDialogOpen} onPrimaryAction={() => {
            setSaveDialogOpen(false);
            saveStagedRows();
        }} open={isSaveDialogOpen} primaryLabel="yes, write to db" prompt={saveConfirmationPrompt} secondaryLabel="no, keep editing" title="Confirm staged row save"/>
      <BinaryAlertDialog onOpenChange={setDiscardDialogOpen} onPrimaryAction={confirmDiscardStagedChanges} open={isDiscardDialogOpen} primaryClassName="border-amber-300 bg-amber-100 text-amber-950 hover:border-amber-400 hover:bg-amber-200" primaryLabel="yes, discard" primaryVariant="outline" prompt={discardConfirmationPrompt} secondaryLabel="no, keep editing" title="Confirm staged edit discard"/>
    </>);
}
function getDeleteSelectionLabel(selectedRowCount) {
    return `Delete ${selectedRowCount} ${selectedRowCount === 1 ? "row" : "rows"}`;
}
function getDeleteConfirmationPrompt(selectedRowCount) {
...
function getSelectionExportColumnIds(args) {
    ...
    const pinnedColumnIds = (columnPinning.left ?? []).filter((columnId) => columnId !== "__ps_select" && validColumnIds.has(columnId));
    const seen = new Set();
...
function createEmptyStagedRowDraft() {
    return {
...
function stripDraftMetadataFromStagedRows(rows) {
    return rows.map((row) => {
        const { [STAGED_ROW_DRAFT_ID_KEY]: _draftId, ...persistableRow } = row;
        return persistableRow;
    });
}
function getEditableColumnIds(columns, readonly) {
...
function isWritableColumn(column, readonly) {
    return !readonly && !column.isAutoincrement && !column.isComputed;
}
function compareColumnsForEditing(left, right) {
...
function BackRelationHeaderCell(props) {
    const { name } = props;
    return (<div className="flex h-full min-w-0 items-center px-2">
      <span className="min-w-0 truncate font-mono text-xs text-foreground/90">
        {name}
      </span>
    </div>);
}
function upsertStagedCellUpdate(stagedUpdates, args) {
...
function upsertStagedRowUpdates(stagedUpdates, updates) {
...
function getSaveStagedRowsLabel(rowCount) {
    return `Save ${rowCount} ${rowCount === 1 ? "row" : "rows"}`;
}
function getDiscardConfirmationPrompt(cellCount) {
    return `Discard edits to ${cellCount} ${cellCount === 1 ? "cell" : "cells"}?`;
}
function getSaveConfirmationPrompt(rowCount) {
    return `Commit ${rowCount} updated ${rowCount === 1 ? "row" : "rows"} to the database?`;
}
function getSaveStagedRowsSuccessMessage(rowCount) {
...
function getStagedCellCount(args) {
    ...
    const stagedUpdateCellCount = stagedUpdates.reduce((count, stagedUpdate) => count + Object.keys(stagedUpdate.changes).length, 0);
    return stagedRowCellCount + stagedUpdateCellCount;
}
function getFocusedCellDirection(key) {
    switch (key) {
        case "ArrowUp":
            return "up";
        case "ArrowDown":
...
function isKeyboardEditableElement(target) {
...
function isGridFocusedCell(value) {
    if (value == null) {
        return true;
    }
...
function BinaryAlertDialog(props) {
    const { onOpenChange, onPrimaryAction, open, primaryClassName, primaryLabel, primaryVariant = "default", prompt, secondaryLabel, title, } = props;
    ...
    return (<AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-[280px] gap-2.5 p-3 font-sans" onKeyDown={(event) => handleBinaryDialogKeyDown({
            event,
            onPrimaryAction,
        ...
        <AlertDialogFooter className="flex-row justify-end gap-1.5">
          <AlertDialogAction ref={primaryButtonRef} size="sm" variant={primaryVariant} className={primaryClassName} onClick={onPrimaryAction}>
            {primaryLabel}
          </AlertDialogAction>
          <AlertDialogCancel ref={secondaryButtonRef} size="sm" variant="secondary" onClick={handleSecondaryAction}>
            {secondaryLabel}
          </AlertDialogCancel>
        </AlertDialogFooter>
      </AlertDialogContent>
...
function ActiveTableRowSearchControl(props) {
    const { disabled = false, onBlockedInteraction, rowSearch, supportsFullTableSearch, } = props;
    const { closeRowSearch, isRowSearchOpen, openRowSearch, rowSearchInputRef, searchInput, setSearchInput, } = rowSearch;
    useEffect(() => {
        if (!disabled) {
            return;
        }
    ...
    function handleBlockedMouseDown(event) {
        if (!disabled) {
            return;
        }
        event.preventDefault();
    ...
    return (<div className={cn("relative h-9 transition-[width] duration-200 ease-out", isRowSearchOpen ? "w-56" : "w-9")} data-row-search-open={isRowSearchOpen ? "true" : "false"}>
      <Button aria-disabled={disabled || undefined} aria-label="Global search" variant="outline" size="icon" className={cn("absolute right-0 top-0 min-w-9 w-auto px-2 transition-opacity duration-200", disabled && "opacity-70", isRowSearchOpen && "opacity-0 pointer-events-none")} onMouseDown={handleBlockedMouseDown} onClick={() => {
            if (disabled) {
                handleBlockedInteraction();
                return;
            }
      ...
      <div data-row-search-input-wrapper className={cn("absolute right-0 top-1/2 -translate-y-1/2 origin-right transition-[opacity,transform] duration-200 ease-out will-change-transform w-56 z-10", isRowSearchOpen
            ? "opacity-100 scale-x-100"
            : "opacity-0 scale-x-0 pointer-events-none")}>
        <Input aria-disabled={disabled || undefined} aria-label="Global search" className={cn("h-9 w-full bg-background shadow-none", disabled && "opacity-70")} onMouseDown={handleBlockedMouseDown} onBlur={(event) => {
            if (disabled) {
                return;
            }
            ...
            event.preventDefault();
            closeRowSearch();
        }} placeholder="Global search" ref={rowSearchInputRef} readOnly={disabled} value={searchInput}/>
      </div>
    </div>);
}
...
function hasMoreRowsToLoad(rowCount, loadedRowCount) {
    if (rowCount === Infinity) {
        return true;
    }
