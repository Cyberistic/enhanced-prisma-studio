...
export function ActiveTableView(_props) {
  ...
  useLayoutEffect(() => {
    if (previousActiveEditorCellKeyRef.current && !activeEditorCellKey) {
      document.querySelector('[data-grid-scroll-container="true"]')?.focus({
        preventScroll: true,
      });
      setGridFocusRequestId((previous) => previous + 1);
    }
  ...
  const columnDefs = [...concreteColumnDefs, ...virtualColumnDefs, ...backRelationColumnDefs];
  if (!activeTable) {
    return null;
  }
  return (
    <>
      ...
      <BinaryAlertDialog
        ...
        primaryClassName="border-amber-300 bg-amber-100 text-amber-950 hover:border-amber-400 hover:bg-amber-200"
