...
export function Studio(props) {
    ...
    return (<StudioContextProvider adapter={adapter} aiFilter={aiFilter} onEvent={onEvent} theme={theme}>
      <StudioContent />
...
function StudioContent() {
    ...
    const { metadata: { activeTable }, viewParam, } = useNavigation();
    ...
    const shouldShowStartupIntrospectionRecovery = viewParam === "table" &&
        activeTable == null &&
        errorState != null &&
        !hasResolvedIntrospection;
    return (<div className="ps" style={{ width: "100%", height: "100%" }}>
      <StudioCommandPaletteProvider>
        <div className={containerClasses}>
          <div className="flex gap-0 bg-background relative min-h-full rounded-lg">
            <AnimatePresence mode="wait">
              {isNavigationOpen && (<motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: "auto", opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.14 }}>
                  <Navigation />
                </motion.div>)}
            </AnimatePresence>

            <motion.div className="flex w-full bg-secondary flex-col p-px rounded-lg self-start h-full min-h-full max-h-full overflow-clip" transition={{ duration: 0.14 }}>
              {shouldShowStartupIntrospectionRecovery ? (<StartupIntrospectionRecoveryView errorState={errorState} isRetrying={isRefetching} onRetry={() => void refetch()}/>) : (<View />)}
            </motion.div>
          </div>
        </div>
      </StudioCommandPaletteProvider>
...
function StartupIntrospectionRecoveryView(props) {
    ...
    return (<div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="flex flex-1 items-center justify-center p-6">
        <IntrospectionStatusNotice className="w-full max-w-2xl" description="Studio could not load schema and table metadata. Retry after checking database permissions or connectivity." isRetrying={isRetrying} message={errorState.message} onRetry={onRetry} queryPreview={errorState.queryPreview} source={errorState.adapterSource} title="Could not load schema metadata"/>
      </div>
    </div>);
}
